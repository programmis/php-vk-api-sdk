...
PHPCBF_BIN=./vendor/squizlabs/php_codesniffer/scripts/phpcbf
...
if [ "$FILES" == ".gitignore" ]; then
    exit 0
fi

if [ "$FILES" == "pre-commit.sh" ]; then
    exit 0
fi
...
do
#  OUTPUT=$($PHPCBF_BIN --standard=$PHPCS_CODING_STANDARD $FILE)
...
if [ $RETVAL -ne 0 ]; then
    echo "$OUTPUT"
    echo "--- Check CodeStandards FAIL ---"
    exit $RETVAL
fi

echo "--- CodeSniffer PASSED ---"
exit 0